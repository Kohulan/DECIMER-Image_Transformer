...
def main():
	num_shards = int(1048576/128) #corresponds to total train files

	get_train_tfrecord(num_shards)
...
def get_train_tfrecord(num_shards):

	train_captions = pickle.load(open("Capts_Path_29360128_30408704.pkl","rb")) #Load the exact Captions path file generated from Create_tokenizer.py
	img_name_vector = pickle.load(open("Images_Path_29360128_30408704.pkl","rb")) #Load the exact Images path file generated from Create_tokenizer.py
	...
	ix = 229376 # Index always corresponds to the previous amount of file generated
	for i in range(num_shards):
		...
		tfrecord_name = 'tfrecords29/'+ 'train-%02d.tfrecord' % ix #path can be changed accordingly
