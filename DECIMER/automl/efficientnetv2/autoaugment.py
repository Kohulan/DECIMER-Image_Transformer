...
def build_and_apply_nas_policy(policies, image, augmentation_params):
        ...
        with tf.name_scope("randaug_layer_{}".format(layer_num)):
            for (i, op_name) in enumerate(available_ops):
                ...
                func, _, args = _parse_policy_info(
                    op_name, prob, random_magnitude, replace_value, augmentation_params
                )
                image = tf.cond(
...
def distort_image(image, aug_name, ra_num_layers, ra_magnitude):
    ...
    if aug_name == "autoaug":
        ...
        image = tf.cond(
            tf.equal(i, 0),
            lambda: distort_image_with_autoaugment(image, "v0"),
            lambda: image,
        )
        image = tf.cond(
            tf.equal(i, 1),
            lambda: distort_image_with_randaugment(image, ra_num_layers, ra_magnitude),
            lambda: image,
        )
    else:
