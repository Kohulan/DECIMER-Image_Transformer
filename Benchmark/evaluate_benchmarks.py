...
def compare_molecules_inchi_match(input_file_path: str, reference_directory: str) -> None:
	"""
		...
		for line in lines:
			...
			if input_mol:
				for format in ['.mol', '.sdf', '.MOL']:
					if os.path.exists(os.path.join(reference_directory, ID[:-4] + format)):
						reference_mol_suppl = Chem.SDMolSupplier(os.path.join(reference_directory, ID[:-4] + format))
					
				# Generate Inchi
				input_inchi = Chem.inchi.MolToInchi(input_mol) #rdmolfiles.MolToSmiles(input_mol, True)
				for reference_mol in reference_mol_suppl:
					if reference_mol:
						reference_inchi = Chem.inchi.MolToInchi(reference_mol)#.rdmolfiles.MolToSmiles(reference_mol, True)#
	...
	print('{} out of {} ({}%)'.format(perfect_match_count, len(lines), perfect_match_count/len(lines)*100))
	print('Average Tanimoto: {}'.format(sum(tanimoto_list)/len(tanimoto_list)))
	tanimoto_one_count = len([tanimoto for tanimoto in tanimoto_list if tanimoto == 1.0])
	print('Tanimoto 1.0 count: {} out of {} ({}%)'.format(tanimoto_one_count, len(lines), tanimoto_one_count/len(lines)))


def main():
	if len(sys.argv) != 3:
		print("\"Usage of this function: {} decimer_benchmark_results_file directory-with-reference-molfiles".format(sys.argv[0]))
	if len(sys.argv) == 3:
